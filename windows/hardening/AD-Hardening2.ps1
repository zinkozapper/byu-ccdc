...
function Change-Passwords {
    ...
    try {
        ...
        # Step c: Get list of AD user accounts
        $UserList = Get-ADUser | Select-Object -ExpandProperty sAMAccountName
        Write-Host "[INFO] Found $($UserList.Count) AD User account(s) to process" -ForegroundColor Cyan
        ...
        foreach ($Username in $UserList) {
            try {
                ...
                Get-ADUser -Name $Username | Set-ADUser -Password $SecurePassword
...
function Print-Users {
    ...
    try {
        ...
        $enabledUsers = Get-ADUser | Where-Object { $_.Enabled -eq $true } | ForEach-Object {
...
function Show-Main-Menu {
    ...
    Write-Host "`n==== DoMain Controller Windows Hardening Menu ====" -ForegroundColor Green
